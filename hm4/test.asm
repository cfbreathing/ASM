...
generate_one_word_mark: ; 标记已经生成的单词
    ...
    MOV     DL, BYTE PTR W[SI]._LETTER[0]
    SUB     DL, 'a'
    MOV     DI, DX
...
generate_one_word_find: ; 找到没出现的单词后，将其显示在屏幕上
    ...
    CALL    get_rand
    MOV     BX, 50H
    SUB     BL, BYTE PTR W[DI]._SIZE
    XOR     AH, AH
    DIV     BL
    MOV     BYTE PTR W[DI]._X, AH
...
show_one_word:  ; 展示 W[i] 的单词
    ...
    MUL     BX
    MOV     BP, AX
    MOV     CL, BYTE PTR W[BP]._HIT_LEN
    MOV     DL, BYTE PTR W[BP]._X
    MOV     DH, BYTE PTR W[BP]._Y
    XOR     SI, SI
show_word_loop:
    MOV     AL, BYTE PTR W[BP]._LETTER[SI]
...
draw_bullet:    ; 画出一颗子弹
    ...
    MUL     BX
    MOV     SI, AX
    MOV     DL, BYTE PTR W[SI]._PX
    MOV     DH, BYTE PTR W[SI]._PY
    MOV     AL, BYTE PTR W[SI]._OLD_CHAR
    MOV     AH, BYTE PTR W[SI]._OLD_COLOR
    CALL    show_char   ; 在上次子弹的坐标(w[i].px, w[i].py)处以w[i].old_color这个颜色重画字符
                        ; w[i].old_char, 即重画被子弹覆盖的字符
    MOV     DL, BYTE PTR W[SI]._BX
    MOV     DH, BYTE PTR W[SI]._BY
    CALL    get_char_color
    MOV     BYTE PTR W[SI]._OLD_CHAR, AL
    MOV     BYTE PTR W[SI]._OLD_COLOR, AH
    MOV     AL, 18H
    MOV     AH, 0CH
    CALL    show_char   ; 在当前子弹坐标处画子弹 
    MOV     BYTE PTR W[SI]._PX, DL
    MOV     BYTE PTR W[SI]._PY, DH
    DEC     DH          ; 当前子弹的 y 坐标减1,表示子弹继续向上移动
    MOV     BYTE PTR W[SI]._BY, DH
    POP     SI
    POP     DX
    POP     AX
    RET
...
int_8h_draw:            ; 找到需要销毁的单词，发射子弹
    ...
    MUL     BX
    MOV     SI, AX
    MOV     DL, BYTE PTR W[SI]._BY
    MOV     CH, BYTE PTR W[SI]._Y
...
main_loop:
    CMP     [STOP], 0H
    JNZ     main_done
    MOV     [TICKS_TO_WAIT], 01H
wait_a_tick:
    ; SUB     [TICKS_TO_WAIT], 1H
    CMP     [TICKS_TO_WAIT], 0H
    JNZ     wait_a_tick
